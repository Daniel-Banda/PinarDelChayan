...
if (item.tipo === 'titulo') {
  ...
  `;
          ...
          return `<small class="d-block f mb-3">${wrapText(item.texto, 60)}</small>`;
        }

        // Items con copa y bot
      if (item.bot !== undefined && (tipo === 'vino' || tipo === 'destilados')) {
        return `
          <div class="mb-3">
            <div class="d-flex justify-content-between">
              <strong style="flex: 1;">${wrapText(item.nombre, 25)}</strong>
              <span style="width: 50px; text-align: right;">${item.copa !== undefined ? `$${item.copa}` : 'N/A'}</span>
